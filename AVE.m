...
load('./mid_data/vd19_V_F_t1_par2_wrelu.mat');
Vtr = Vtr1;
Vts = Vts1;
clear Vtr1 Vts1;
num_tr = size(Vtr,1);
...
for i = 1:num_tr
    tr_fea(i,:) = mean(Vtr{i},2); 
end
for i = 1:num_ts
    ts_fea(i,:) = mean(Vts{i},2); 
end
[~,tr_label] = max(Ftr,[],2);
[~,ts_label] = max(Fts,[],2);

%save('./mid_data/par3_task1_ave_trts_fea_label.mat','tr_fea','ts_fea','tr_label','ts_label','-v7.3');
...
accuracy1 = mean(acc);
fprintf('Arage Class accuracy for flower17: %f\n',accuracy1 );

accuracy2 = length(find(ts_label == C))/length(C);
fprintf('Arage Classification accuracy for flower17: %f\n',accuracy2 );
